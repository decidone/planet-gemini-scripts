...
게임 진행 중 접속한 클라이언트에게 대량의 오브젝트(건물, 벨트, 포탈 등)의 상태를 전달할 때, 단일 프레임에 대량의 RPC를 집중 발송하면 호스트에서는 전송이 완료되어도 클라이언트가 일부를 수신하지 못하는 문제가 발생합니다. 별도의 오류 없이 누락되는 것으로 보아, 한 번에 처리할 수 있는 양을 넘어선 RPC가 폐기된 것으로 판단했습니다. 이를 방지하기 위해 상태 데이터를 배치 단위로 분할 전송하고, 수신 확인(ACK)을 받지 못한 미확인 배치를 일정 수 이하로 유지하는 슬라이딩 윈도우 알고리즘을 구현하여 데이터 전송량을 제어했습니다.

관련 스크립트: [`NetworkObjManager.cs`](./NetworkObjManager.cs)
...
- **슬라이딩 윈도우 기반 전송량 제어:** 상태 동기화 RPC를 배치 단위로 분할하고 미확인 배치 수를 최대 5개로 제한해 클라이언트 접속 시점의 RPC 누락으로 인한 접속 장애를 방지하도록 구현했습니다.
