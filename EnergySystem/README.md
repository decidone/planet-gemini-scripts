...
에너지 관련 건물마다 부여된 원형 커버 영역이 겹치면 하나의 전력망(그룹)으로 병합하고, 발전량과 소비량의 비율로 산출된 효율을 소속 건물의 생산 속도에 반영합니다. 커넥터 연결이 끊길 때는 시그널 전파(DFS 플러드 필)로 도달 가능한 노드를 판정하여 신호가 닿지 않는 커넥터들을 별도 전력망으로 분리하며, `LDConnector`를 활용해 물리적으로 떨어진 거점 전력망도 단일망으로 통합할 수 있도록 구현했습니다.
